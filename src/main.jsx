...
const [spreadsheetExpanded, setSpreadsheetExpanded] = useState(null);

if (!unlocked) return <PasswordGate onUnlock={unlock} />;

const printStyles = `@media print { .no-print { display: none !important; } body { font-family: Georgia, serif; } }`;
...
<button onClick={() => setView(“spreadsheet”)} style={btnStyle(”#5a7a4e”, “#fff”)}>⊞ Spreadsheet</button>
<button onClick={() => setView(“print”)} style={btnStyle(”#2c6a8a”, “#fff”)}>🖨 Print</button>
</div>
</div>
  ...
  <div style={{ maxWidth: 760, margin: "0 auto", padding: "24px 16px" }}>
    ...
    {view === "detail" && selectedAnimal && (
      <div>
        ...
        {/* Print & Delete */}
        <div style={{ marginTop: 32, borderTop: "1px solid #ddd6c4", paddingTop: 20, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <button onClick={() => window.print()} style={btnStyle("#2c6a8a", "#fff")}>🖨 Print Record</button>
        </div>
    ...
    {view === "spreadsheet" && (
      <div>
        <div style={{ fontSize: 20, fontWeight: "bold", marginBottom: 4 }}>Spreadsheet View</div>
        <div style={{ fontSize: 13, color: "#7a6a55", marginBottom: 20 }}>All animals · click a row to expand detail</div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: "Georgia, serif" }}>
            <thead>
              <tr style={{ background: "#2c2416", color: "#f5f0e8" }}>
                {["Tag","Name","DOB","Age","Sex","Gen","Bloodline","Sire","Dam","Latest Wt","Brand Cert"].map(h => (
                  <th key={h} style={{ padding: "8px 10px", textAlign: "left", whiteSpace: "nowrap", fontWeight: "bold", fontSize: 11, letterSpacing: 1 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {cattle.map((a, i) => {
                const lw = a.weights.length ? [...a.weights].sort((x,y) => new Date(y.date)-new Date(x.date))[0].weight + " lbs" : "—";
                const name = a.notes.split(".")[0] !== a.tag ? (a.tag === "#203" ? "Musashi" : a.tag === "#209" ? "Otsu" : a.tag.replace("#","")) : "";
                return (
                  <tr key={a.id} onClick={() => { setSelected(a.id); setSpreadsheetExpanded(a.id === spreadsheetExpanded ? null : a.id); }}
                    style={{ background: i % 2 === 0 ? "#fff" : "#f9f5ee", cursor: "pointer", borderBottom: "1px solid #ddd6c4" }}
                    onMouseEnter={e => e.currentTarget.style.background = "#f0e8d8"}
                    onMouseLeave={e => e.currentTarget.style.background = i % 2 === 0 ? "#fff" : "#f9f5ee"}
                  >
                    <td style={{ padding: "8px 10px", fontWeight: "bold" }}>{a.tag}</td>
                    <td style={{ padding: "8px 10px" }}>{name}</td>
                    <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>{formatDate(a.dob)}</td>
                    <td style={{ padding: "8px 10px", whiteSpace: "nowrap" }}>{ageStr(a.dob)}</td>
                    <td style={{ padding: "8px 10px" }}>{a.sex}</td>
                    <td style={{ padding: "8px 10px" }}>{a.generation}</td>
                    <td style={{ padding: "8px 10px" }}>{a.bloodline}</td>
                    <td style={{ padding: "8px 10px" }}>{a.sire || "—"}</td>
                    <td style={{ padding: "8px 10px" }}>{a.dam || "—"}</td>
                    <td style={{ padding: "8px 10px", fontWeight: "bold" }}>{lw}</td>
                    <td style={{ padding: "8px 10px" }}>{a.brandCert || "—"}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        {/* Expanded Detail */}
        {spreadsheetExpanded && (() => {
          const a = cattle.find(c => c.id === spreadsheetExpanded);
          if (!a) return null;
          return (
            <div style={{ background: "#fff", border: "2px solid #2c2416", borderRadius: 10, padding: 20, marginBottom: 24 }}>
              <div style={{ fontWeight: "bold", fontSize: 16, marginBottom: 16, borderBottom: "1px solid #ddd6c4", paddingBottom: 8 }}>{a.tag} — Detail</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
                <div>
                  <div style={{ fontSize: 11, fontWeight: "bold", letterSpacing: 2, textTransform: "uppercase", color: "#2c2416", borderBottom: "1px solid #ddd6c4", paddingBottom: 4, marginBottom: 10 }}>Weight History</div>
                  {a.weights.length === 0 ? <div style={{ color: "#a89070", fontSize: 12 }}>No records</div>
                    : [...a.weights].sort((x,y) => new Date(y.date)-new Date(x.date)).map((w,i) => (
                      <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f0ebe0", display: "flex", gap: 12 }}>
                        <span style={{ fontWeight: 600 }}>{w.weight} lbs</span>
                        <span style={{ color: "#7a6a55" }}>{formatDate(w.date)}</span>
                        <span style={{ color: "#a89070" }}>{w.event}</span>
                      </div>
                    ))
                  }
                </div>
                <div>
                  <div style={{ fontSize: 11, fontWeight: "bold", letterSpacing: 2, textTransform: "uppercase", color: "#2c2416", borderBottom: "1px solid #ddd6c4", paddingBottom: 4, marginBottom: 10 }}>Vaccinations</div>
                  {a.vaccinations.length === 0 ? <div style={{ color: "#a89070", fontSize: 12 }}>No records</div>
                    : [...a.vaccinations].sort((x,y) => new Date(y.date)-new Date(x.date)).map((v,i) => (
                      <div key={i} style={{ fontSize: 12, padding: "4px 0", borderBottom: "1px solid #f0ebe0", display: "flex", gap: 12, flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 600 }}>{v.product}</span>
                        <span style={{ color: "#7a6a55" }}>{formatDate(v.date)}</span>
                        <span style={{ color: "#a89070" }}>{v.mfg}</span>
                      </div>
                    ))
                  }
                </div>
              </div>
              {a.notes && <div style={{ marginTop: 12, fontSize: 12, color: "#5a4a35", borderTop: "1px solid #ddd6c4", paddingTop: 10 }}><strong>Notes:</strong> {a.notes}</div>}
            </div>
          );
        })()}
      </div>
    ...
    {view === "print" && (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }} className="no-print">
          <div style={{ fontSize: 20, fontWeight: "bold" }}>Print Preview</div>
          <button onClick={() => window.print()} style={btnStyle("#2c2416", "#f5f0e8")}>🖨 Print / Save PDF</button>
        </div>

        <div id="print-area">
          {/* Print Header */}
          <div style={{ textAlign: "center", borderBottom: "3px solid #2c2416", paddingBottom: 16, marginBottom: 24 }}>
            <div style={{ fontSize: 24, fontWeight: "bold", letterSpacing: 1 }}>V2 Ranch — Cattle Records</div>
            <div style={{ fontSize: 12, color: "#7a6a55", marginTop: 4 }}>Printed {new Date().toLocaleDateString()}</div>
          </div>

          {/* Herd Summary Table */}
          <div style={{ marginBottom: 32 }}>
            <div style={{ fontSize: 14, fontWeight: "bold", letterSpacing: 2, textTransform: "uppercase", borderBottom: "2px solid #2c2416", paddingBottom: 6, marginBottom: 12 }}>Herd Summary</div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead>
                <tr style={{ background: "#2c2416", color: "#f5f0e8" }}>
                  {["Tag","DOB","Sex","Generation","Bloodline","Sire","Dam","Latest Wt","Brand Cert"].map(h => (
                    <th key={h} style={{ padding: "6px 8px", textAlign: "left", fontSize: 10, letterSpacing: 0.5 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {cattle.map((a, i) => {
                  const lw = a.weights.length ? [...a.weights].sort((x,y) => new Date(y.date)-new Date(x.date))[0].weight + " lbs" : "—";
                  return (
                    <tr key={a.id} style={{ background: i % 2 === 0 ? "#fff" : "#f9f5ee", borderBottom: "1px solid #ddd6c4" }}>
                      <td style={{ padding: "6px 8px", fontWeight: "bold" }}>{a.tag}</td>
                      <td style={{ padding: "6px 8px" }}>{formatDate(a.dob)}</td>
                      <td style={{ padding: "6px 8px" }}>{a.sex}</td>
                      <td style={{ padding: "6px 8px" }}>{a.generation}</td>
                      <td style={{ padding: "6px 8px" }}>{a.bloodline}</td>
                      <td style={{ padding: "6px 8px" }}>{a.sire || "—"}</td>
                      <td style={{ padding: "6px 8px" }}>{a.dam || "—"}</td>
                      <td style={{ padding: "6px 8px", fontWeight: "bold" }}>{lw}</td>
                      <td style={{ padding: "6px 8px" }}>{a.brandCert || "—"}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          {/* Individual Records */}
          {cattle.map(a => (
            <div key={a.id} style={{ marginBottom: 28, pageBreakInside: "avoid", borderTop: "2px solid #2c2416", paddingTop: 16 }}>
              <div style={{ fontSize: 15, fontWeight: "bold", marginBottom: 12 }}>{a.tag} {a.notes.startsWith(a.tag.replace("#","")) ? "" : ""}</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "8px 24px", fontSize: 11, marginBottom: 12 }}>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>DOB</span><br/>{formatDate(a.dob)}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Sex</span><br/>{a.sex}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Generation</span><br/>{a.generation}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Bloodline</span><br/>{a.bloodline}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Sire</span><br/>{a.sire || "—"}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Dam</span><br/>{a.dam || "—"}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>AWA Reg</span><br/>{a.awaReg || "—"}</div>
                <div><span style={{ color: "#a89070", textTransform: "uppercase", fontSize: 9, letterSpacing: 1 }}>Brand Cert</span><br/>{a.brandCert || "—"}</div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, fontSize: 11 }}>
                <div>
                  <div style={{ fontWeight: "bold", fontSize: 10, letterSpacing: 1, textTransform: "uppercase", borderBottom: "1px solid #ddd6c4", paddingBottom: 3, marginBottom: 6 }}>Weight History</div>
                  {a.weights.length === 0 ? <div style={{ color: "#a89070" }}>—</div>
                    : [...a.weights].sort((x,y) => new Date(y.date)-new Date(x.date)).map((w,i) => (
                      <div key={i} style={{ display: "flex", gap: 10, padding: "2px 0", borderBottom: "1px solid #f0ebe0" }}>
                        <span style={{ fontWeight: 600, minWidth: 60 }}>{w.weight} lbs</span>
                        <span style={{ color: "#7a6a55", minWidth: 80 }}>{formatDate(w.date)}</span>
                        <span style={{ color: "#a89070" }}>{w.event}</span>
                      </div>
                    ))
                  }
                ...
                <div>
                  <div style={{ fontWeight: "bold", fontSize: 10, letterSpacing: 1, textTransform: "uppercase", borderBottom: "1px solid #ddd6c4", paddingBottom: 3, marginBottom: 6 }}>Vaccinations</div>
                  {a.vaccinations.length === 0 ? <div style={{ color: "#a89070" }}>—</div>
                    : [...a.vaccinations].sort((x,y) => new Date(y.date)-new Date(x.date)).map((v,i) => (
                      <div key={i} style={{ display: "flex", gap: 10, padding: "2px 0", borderBottom: "1px solid #f0ebe0", flexWrap: "wrap" }}>
                        <span style={{ fontWeight: 600 }}>{v.product}</span>
                        <span style={{ color: "#7a6a55" }}>{formatDate(v.date)}</span>
                      </div>
                    ))
              ...
              {a.notes && <div style={{ marginTop: 8, fontSize: 10, color: "#5a4a35" }}><strong>Notes:</strong> {a.notes}</div>}
            </div>
          ))}
        </div>
      </div>
